            ...
            if (!string.IsNullOrEmpty(_packet.Message))
            {
                ...
                else
                {
                    if (_user.GroupReceiveMessages.All(c => c.Sequence != _packet.Sequence))
                    {
                        ...
                        {
                            _service.MessageLog($"收到群{_packet.Group}的{_packet.FromQQ}的乱码消息。", MsgType.INFO);
                        }

                        _service.MessageLog($"收到群{_packet.Group}的{_packet.FromQQ}的消息:{_packet.Message}", MsgType.INFO);
